...
public class CryptoServiceImpl implements ICryptoService {
    ...
    @Value("${mrs.sm2-private-key}")
    private String SM2PrivateKey;

    @Override
    public String getPublicKey(String fingerPrint) {
        String publicKey = redisTemplate.opsForValue().get(fingerPrint + TypeSuffix.CKP.getTypeSuffix()) + "";
        if (StringUtils.isNotBlank(publicKey)) {
            return publicKey;
        }
//        SM2KeyPair skp = SM2Utils.generateKeyPair();
//        redisTemplate.opsForValue().set(fingerPrint + TypeSuffix.SKP.getTypeSuffix(), skp, KEYPAIR_EXPIRE_TIME, TimeUnit.SECONDS);
